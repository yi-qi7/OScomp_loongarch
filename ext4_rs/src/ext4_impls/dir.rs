...
impl Ext4 {
    ...
    pub fn dir_find_in_block(
        ...
        while offset < BLOCK_SIZE - core::mem::size_of::<Ext4DirEntryTail>() {
            ...
            //发现de.entry_len() 的值全为0
            log::error!("entry_len:{}",de.entry_len() as usize);
        }
        log::error!("end_dir_find_in_block");
    ...
    pub fn dir_set_csum(&self, dst_blk: &mut Block, ino_gen: u32) {
        ...
        log::info!("dir_set_csum");

        tail.copy_to_slice(&mut dst_blk.data);
        log::info!("dir_set_csum 2");
    ...
    pub fn dir_add_entry(
        ...
        // // log::info!("reach check point yusan");
        // log::info!("addr of self.block_device: {:p}", Arc::as_ptr(&self.block_device));
        // log::info!("addr of self.block_device: {:p}", Arc::as_ptr(&self.block_device));
        let test_clone = self.block_device.clone();
        log::info!("addr of test_clone : {:p}", Arc::as_ptr(&test_clone));
        // set checksum
